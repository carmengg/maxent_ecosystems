...
#readr::write_csv(pts_train,here("oaks","oaks_data","oaks_train.csv"))
#readr::write_csv(pts_test,here("oaks","oaks_data","oaks_test.csv"))


```

```{r}
pts_train<-read_csv(here("oaks","oaks_data","oaks_train.csv"))
pts_test<-read_csv(here("oaks","oaks_data","oaks_test.csv"))
```
...
glm_prediction_raster <- raster::predict(env_stack, 
                                         log_model, 
                                         type="response")
...
ROCRpred <- ROCR::prediction(test_probabilities_2$value,
                             test_probabilities_2$presence)
...
```{r}
thr <- threshold(e_test)[['spec_sens']]
